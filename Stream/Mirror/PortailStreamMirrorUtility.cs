...
namespace Portail.Stream.Mirror
{
	...
	internal static class PortailStreamMirrorUtility
	{
		public static void RefreshPlayerViews(
			Dictionary<int, MirrorPortailParticipant> players,
			HashSet<int> livePlayerIds,
			List<int> stalePlayerIds,
			out MirrorPortailParticipant localSource)
		{
			livePlayerIds.Clear();
			localSource = null;
			...
			if (NetworkClient.spawned != null)
			{
				foreach (KeyValuePair<uint, NetworkIdentity> entry in NetworkClient.spawned)
				{
					...
					if (identity == null || !identity.TryGetComponent(out MirrorPortailParticipant source) || source == null)
						continue;
			...
			foreach (KeyValuePair<int, MirrorPortailParticipant> pair in players)
			{
				int id = pair.Key;
