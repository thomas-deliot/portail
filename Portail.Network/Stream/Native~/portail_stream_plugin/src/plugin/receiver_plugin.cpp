...
#include "receiver/receiver_audio_pipeline.h"
#include "receiver/receiver_connection_state.h"
#include "receiver/receiver_peer_state.h"
#include "receiver/receiver_protocol_sender.h"
#include "receiver/receiver_quality_state.h"
...
#include "receiver/receiver_video_state.h"
#include "plugin/receiver_plugin_api.h"
...
namespace
{
	...
	using streamproto::receiver::ReceiverMaxAcceptedQuality;
	using streamproto::receiver::ReceiverAudioPipeline;
	using streamproto::receiver::ReceiverConnectionState;
	using streamproto::receiver::PeerExportState;
	using streamproto::receiver::UnityOutputBinding;
	using streamproto::receiver::ReceiverPeerQualityState;
	using streamproto::receiver::BuildReceiverStatsLine;
	using streamproto::receiver::ReceiverStats;
	using streamproto::receiver::ConfiguredVideoLod;
	using streamproto::receiver::FfmpegDecoder;
	using streamproto::receiver::ReceiverVideoDecodeEnqueueResult;
	using streamproto::receiver::ReceiverVideoDecodeEvent;
	using streamproto::receiver::ReceiverVideoState;
	using streamproto::receiver::VideoDecoderConfigKey;
	using streamproto::receiver::InspectRecoveryUnit;
	using streamproto::receiver::RecoveryUnitInfo;
	using streamproto::receiver::SendReceiverAppliedQuality;
	using streamproto::receiver::SendReceiverMaxAcceptedQuality;
	using streamproto::receiver::SendKeyframeRequest;
	using streamproto::receiver::SendPing;
	using streamproto::receiver::SendStreamControl;
	using streamproto::receiver::StreamConfigKey;
	namespace proto = streamproto::proto;

	constexpr int kReceiverRenderEventId = 0x1002;

	std::mutex g_error_mutex;
	std::string g_last_error;
	using ReceiverLogCallback = streamproto::log::Callback;
	std::mutex g_log_callback_mutex;
	ReceiverLogCallback g_log_callback = nullptr;
	std::atomic<bool> g_receiver_logging_enabled{true};
	...
	void SetReceiverLogCallback(ReceiverLogCallback callback)
	{
		std::lock_guard<std::mutex> lock(g_log_callback_mutex);
		g_log_callback = callback;
	}

	ReceiverLogCallback GetReceiverLogCallback()
	{
		std::lock_guard<std::mutex> lock(g_log_callback_mutex);
		return g_log_callback;
	}

	void SetReceiverLoggingEnabled(bool enabled)
	{
		g_receiver_logging_enabled.store(enabled);
	}

	streamproto::log::Stream LogInfoStream()
	{
		return streamproto::log::Stream(std::cout, streamproto::log::Level::kInfo, GetReceiverLogCallback(), g_receiver_logging_enabled.load());
	}

	streamproto::log::Stream LogWarningStream()
	{
		return streamproto::log::Stream(std::cerr, streamproto::log::Level::kWarning, GetReceiverLogCallback(), g_receiver_logging_enabled.load());
	}

	streamproto::log::Stream LogErrorStream()
	{
		return streamproto::log::Stream(std::cerr, streamproto::log::Level::kError, GetReceiverLogCallback(), g_receiver_logging_enabled.load());
	}
	...
	void PrintStatsLine(
		...
		ReceiverStats &last_snapshot,
		FILE *stats_log_file)
	{
		const std::string text = BuildReceiverStatsLine(stats, decoder_name, path_name, connected, delta_ms, last_snapshot);
...
namespace
{
	...
	std::mutex g_receiver_thread_mutex;
	std::atomic<bool> g_receiver_thread_running{false};
	std::atomic<bool> g_receiver_stop_requested{false};
	std::atomic<bool> g_receiver_pause_requested{false};
	std::atomic<std::uint64_t> g_receiver_local_steam_id{0};
	std::mutex g_desired_senders_mutex;
	std::vector<std::uint64_t> g_desired_senders;
	std::atomic<bool> g_desired_senders_dirty{false};
	std::mutex g_max_accepted_qualities_mutex;
	std::unordered_map<std::uint64_t, ReceiverMaxAcceptedQuality> g_max_accepted_qualities;
	std::atomic<bool> g_max_accepted_qualities_dirty{false};
	std::mutex g_receiver_video_lod_configs_mutex;
	std::vector<ConfiguredVideoLod> g_receiver_video_lod_configs;
	std::atomic<bool> g_receiver_video_lod_configs_dirty{false};
	...
	std::vector<std::uint64_t> CopyDesiredSenders()
	{
		std::lock_guard<std::mutex> lock(g_desired_senders_mutex);
		return g_desired_senders;
	}

	std::unordered_map<std::uint64_t, ReceiverMaxAcceptedQuality> CopyMaxAcceptedQualities()
	{
		std::lock_guard<std::mutex> lock(g_max_accepted_qualities_mutex);
		return g_max_accepted_qualities;
	}

	std::vector<ConfiguredVideoLod> CopyReceiverVideoLodConfigs()
	{
		std::lock_guard<std::mutex> lock(g_receiver_video_lod_configs_mutex);
		return g_receiver_video_lod_configs;
	}
	...
	std::uint64_t FirstDesiredSender()
	{
		std::lock_guard<std::mutex> lock(g_desired_senders_mutex);
		return g_desired_senders.empty() ? 0 : g_desired_senders.front();
	}

	class ReceiverPeerRunner
	{
	public:
		ReceiverPeerRunner(
			std::uint32_t app_id,
			...
			bool disable_ice,
			std::vector<ConfiguredVideoLod> video_lod_configs) : app_id_(app_id), sender_id_(sender_id), disable_ice_(disable_ice)
		{
			video_.configured_lods = std::move(video_lod_configs);
		}
		~ReceiverPeerRunner() { Shutdown(); }

		void SetMaxAcceptedQuality(const ReceiverMaxAcceptedQuality &quality)
		{
		...
		bool Initialize(std::string &error)
		{
			...
			SdrConfig c{};
			c.app_id = app_id_;
			c.role = SdrRole::kReceiver;
			c.remote_steam_id = sender_id_;
			...
			std::shared_ptr<streamproto::audio::PcmRingBuffer> audio_ring = audio_pipeline_.EnsureRing();
			{
				...
				g_peer_audio_rings[sender_id_] = audio_ring;
			}
		...
		void Shutdown()
		{
			...
			std::lock_guard<std::mutex> lock(peer_mutex_);
			{
				...
				g_peer_audio_rings.erase(sender_id_);
			}
			if (connection_.sdr != nullptr)
			{
				connection_.sdr->Close("receiver-peer-shutdown");
				connection_.sdr.reset();
			}
			...
			connection_.sender_paused = false;
			connection_.receiver_pause_sent = false;
		...
		void Tick(bool pause_now)
		{
			...
			if (connected)
			{
				...
				if (pause_now != connection_.receiver_pause_sent)
				{
					SendStreamControl(*connection_.sdr,
									  pause_now ? proto::StreamControlCommand::kReceiverPause : proto::StreamControlCommand::kReceiverResume,
									  connection_.sdr->LocalSteamId());
					connection_.receiver_pause_sent = pause_now;
				}
				if (quality_.NeedsMaxAcceptedSend())
				{
					if (SendReceiverMaxAcceptedQuality(*connection_.sdr, quality_.max_accepted, connection_.sdr->LocalSteamId()))
					{
			...
			if (connected && video_.has_stream_config && video_.waiting_for_keyframe && !pause_now && !connection_.sender_paused)
			{
				std::uint64_t now_ms = streamproto::NowSteadyMillis();
			...
			{
				std::string d = video_.active_decoder + "@" + std::to_string(sender_id_);
				ReceiverStats stats_snapshot = SnapshotStats();
		...
		PeerExportState Export() const
		{
			...
			const ReceiverStats snapshot = SnapshotStats();
			...
			s.paused_by_sender = connection_.sender_paused ? 1 : 0;
	...
	private:
		void PeerWorkerLoop()
		{
			while (!peer_worker_stop_.load(std::memory_order_acquire) &&
				   !g_receiver_stop_requested.load(std::memory_order_acquire))
			{
				{
					std::lock_guard<std::mutex> lock(peer_mutex_);
					Tick(g_receiver_pause_requested.load(std::memory_order_relaxed));
				}
		...
		{
			video_.decode_queue.Start([this](ReceiverVideoDecodeEvent event)
		...
		void EnqueueVideoDecodeEvent(ReceiverVideoDecodeEvent event)
		{
			const ReceiverVideoDecodeEnqueueResult result = video_.decode_queue.Enqueue(std::move(event));
		...
		void ProcessVideoDecodeEvent(ReceiverVideoDecodeEvent event)
		{
			...
			{
				std::int64_t sender_done_us = static_cast<std::int64_t>(streamproto::NowUnixMicros());
				if (connection_.clock_sync.valid)
				{
					sender_done_us += static_cast<std::int64_t>(std::llround(connection_.clock_sync.offset_us));
				}
				if (sender_done_us >= 0 && static_cast<std::uint64_t>(sender_done_us) >= event.capture_ts_us)
				{
					double total_ms = static_cast<double>(static_cast<std::uint64_t>(sender_done_us) - event.capture_ts_us) / 1000.0;
		...
		ReceiverStats SnapshotStats() const
		{
			...
			const proto::AudioConfigMessage &cfg)
		{
			...
			{
				LogWarningStream() << "path=audio peer=" << sender_id_
		...
		void HandlePacket(const std::uint8_t *data, std::size_t bytes, bool pause_now)
		{
			...
			if (type == proto::MessageType::kPong)
			{
				...
				std::uint64_t t0 = m->receiver_send_timestamp_us;
				std::uint64_t t1 = m->sender_recv_timestamp_us;
				std::uint64_t t2 = m->sender_send_timestamp_us;
				if (t3 < t0 || t2 < t1)
					return;
			...
			if (type == proto::MessageType::kStreamControl)
			{
				...
				if (cmd == proto::StreamControlCommand::kSenderPause)
					connection_.sender_paused = true;
				if (cmd == proto::StreamControlCommand::kSenderResume)
					connection_.sender_paused = false;
				return;
			}
			...
			if (type == proto::MessageType::kStreamQuality)
			{
				...
				if (command == proto::StreamQualityCommand::kSenderAssigned)
				{
			...
			{
				std::int64_t sender_us = static_cast<std::int64_t>(streamproto::NowUnixMicros());
				if (connection_.clock_sync.valid)
					sender_us += static_cast<std::int64_t>(std::llround(connection_.clock_sync.offset_us));
				if (sender_us >= 0 && static_cast<std::uint64_t>(sender_us) >= assembled->capture_ts_us)
				{
					reassembly_ms = static_cast<double>(static_cast<std::uint64_t>(sender_us) - assembled->capture_ts_us) / 1000.0;
				}
			}
			...
			ReceiverVideoDecodeEvent decode_event{};
		...
		ReceiverConnectionState connection_{};
		ReceiverVideoState video_{};
		ReceiverAudioPipeline audio_pipeline_{};
		std::unordered_map<std::uint64_t, proto::AudioConfigMessage> audio_configs_by_key_;
		ReceiverStats stats_{};
		ReceiverStats snapshot_{};
		std::uint64_t last_stats_ms_ = 0;
		ReceiverPeerQualityState quality_{};
	};

	void RunReceiverPluginWorker(std::uint32_t app_id, bool disable_ice)
	{
		std::unordered_map<std::uint64_t, std::unique_ptr<ReceiverPeerRunner>> peers;
		std::unordered_map<std::uint64_t, std::uint64_t> retry_after_ms;
		std::unordered_map<std::uint64_t, ReceiverMaxAcceptedQuality> max_accepted_qualities = CopyMaxAcceptedQualities();
		std::vector<ConfiguredVideoLod> video_lod_configs = CopyReceiverVideoLodConfigs();
		std::uint64_t last_reconcile_ms = 0;
		while (!g_receiver_stop_requested.load())
		{
			std::uint64_t now_ms = streamproto::NowSteadyMillis();
			SteamSdr::PumpGlobalCallbacks();
			if (g_receiver_video_lod_configs_dirty.exchange(false))
			{
				video_lod_configs = CopyReceiverVideoLodConfigs();
			...
			if (g_max_accepted_qualities_dirty.exchange(false))
			{
				...
				for (auto &[sender_id, peer] : peers)
				{
					auto quality_it = max_accepted_qualities.find(sender_id);
					peer->SetMaxAcceptedQuality(quality_it != max_accepted_qualities.end()
													? quality_it->second
													: ReceiverMaxAcceptedQuality{});
				}
			}
			bool reconcile = g_desired_senders_dirty.exchange(false) || (now_ms - last_reconcile_ms >= 1000);
			if (reconcile)
			{
				last_reconcile_ms = now_ms;
				std::vector<std::uint64_t> desired = CopyDesiredSenders();
				...
				for (std::uint64_t sender_id : desired_set)
				{
					if (sender_id == 0 || peers.find(sender_id) != peers.end())
						continue;
					auto rt = retry_after_ms.find(sender_id);
					if (rt != retry_after_ms.end() && now_ms < rt->second)
						continue;
					auto peer = std::make_unique<ReceiverPeerRunner>(app_id, sender_id, disable_ice, video_lod_configs);
					std::string err;
					if (!peer->Initialize(err))
					{
						retry_after_ms[sender_id] = now_ms + 5000;
						SetLastErrorString("Failed to initialize receiver peer " + std::to_string(sender_id) + ": " + err);
						continue;
					}
					auto quality_it = max_accepted_qualities.find(sender_id);
					peer->SetMaxAcceptedQuality(quality_it != max_accepted_qualities.end()
					...
					{
						retry_after_ms[sender_id] = now_ms + 5000;
						SetLastErrorString("Failed to start receiver peer worker " + std::to_string(sender_id));
						continue;
					}
					if (g_receiver_local_steam_id.load() == 0 && peer->LocalSteamId() != 0)
						g_receiver_local_steam_id.store(peer->LocalSteamId());
					peers.emplace(sender_id, std::move(peer));
					retry_after_ms.erase(sender_id);
				}
			}
			std::vector<std::uint64_t> reconnect_senders;
			reconnect_senders.reserve(peers.size());
			for (auto &[sender, peer] : peers)
			{
				if (peer->NeedsReconnect())
				{
					reconnect_senders.push_back(sender);
				}
			}
			if (!reconnect_senders.empty())
			{
				for (std::uint64_t sender_id : reconnect_senders)
				{
					peers.erase(sender_id);
					retry_after_ms[sender_id] = now_ms + 1000;
				}
			}
			std::unordered_map<std::uint64_t, PeerExportState> exports;
			exports.reserve(peers.size());
			for (auto &[sender, peer] : peers)
				exports.emplace(sender, peer->Export());
			{
				std::lock_guard<std::mutex> lock(g_peer_exports_mutex);
				for (auto &[sender, next] : exports)
				{
					auto old_it = g_peer_exports.find(sender);
		...
		g_receiver_local_steam_id.store(0);
	}

	void UNITY_INTERFACE_API OnReceiverGraphicsDeviceEvent(UnityGfxDeviceEventType event_type)
	{
	...
	struct ReceiverRenderSourceInfo
	{
	...
	void UNITY_INTERFACE_API OnReceiverRenderEvent(int event_id)
	{
		if (event_id != kReceiverRenderEventId)
		{
			return;
		}
		std::unordered_map<std::uint64_t, ReceiverRenderSourceInfo> sources;
		{
			std::lock_guard<std::mutex> lock(g_peer_exports_mutex);
			sources.reserve(g_peer_exports.size());
			for (const auto &[sender, s] : g_peer_exports)
			{
				if (s.shared_handle != 0)
				{
					ReceiverRenderSourceInfo info{};
		...
		for (auto &[sender, binding] : g_unity_output_bindings)
		{
			if (binding.target_texture == nullptr)
				continue;
			auto it = sources.find(sender);
			if (it == sources.end())
				continue;
			const ReceiverRenderSourceInfo &source_info = it->second;
			...
			if (copied)
			{
				...
				{
					std::int64_t sender_now_us = static_cast<std::int64_t>(streamproto::NowUnixMicros());
					sender_now_us += static_cast<std::int64_t>(std::llround(source_info.clock_offset_ms * 1000.0));
					if (sender_now_us >= 0 && static_cast<std::uint64_t>(sender_now_us) >= source_info.capture_ts_us)
					{
						capture_to_texture_ms = static_cast<double>(static_cast<std::uint64_t>(sender_now_us) - source_info.capture_ts_us) / 1000.0;
					}
				}
				std::lock_guard<std::mutex> export_lock(g_peer_exports_mutex);
				auto export_it = g_peer_exports.find(sender);
...
extern "C"
{
	...
	UNITY_INTERFACE_EXPORT void UNITY_INTERFACE_API UnityPluginLoad(IUnityInterfaces *unity_interfaces)
	{
		...
		{
			g_unity_graphics->RegisterDeviceEventCallback(OnReceiverGraphicsDeviceEvent);
		}
		OnReceiverGraphicsDeviceEvent(kUnityGfxDeviceEventInitialize);
	}
	...
	UNITY_INTERFACE_EXPORT void UNITY_INTERFACE_API UnityPluginUnload()
	{
		if (g_unity_graphics != nullptr)
		{
			g_unity_graphics->UnregisterDeviceEventCallback(OnReceiverGraphicsDeviceEvent);
		}
		SetReceiverLogCallback(nullptr);
		SetReceiverLoggingEnabled(true);
		OnReceiverGraphicsDeviceEvent(kUnityGfxDeviceEventShutdown);
		g_unity_interfaces = nullptr;
		g_unity_graphics = nullptr;
	}

	UNITY_INTERFACE_EXPORT void SSPR_SetLogCallback(ReceiverLogCallback callback)
	{
		SetReceiverLogCallback(callback);
	}

	UNITY_INTERFACE_EXPORT void SSPR_SetLoggingEnabled(bool enabled)
	{
		SetReceiverLoggingEnabled(enabled);
	}

	UNITY_INTERFACE_EXPORT bool SSPR_Start(const SSPR_StartParams *params)
	{
		...
		std::lock_guard<std::mutex> lock(g_receiver_thread_mutex);
		if (g_receiver_thread_running.load())
		{
			return false;
		}

		if (params->sender_steam_id != 0)
		{
			std::lock_guard<std::mutex> dlock(g_desired_senders_mutex);
			if (g_desired_senders.empty())
				g_desired_senders.push_back(params->sender_steam_id);
		}
		g_desired_senders_dirty.store(true);
		g_receiver_stop_requested.store(false);
		g_receiver_pause_requested.store(false);
		g_receiver_local_steam_id.store(0);
		SetLastErrorString("");
		{
		...
		g_receiver_thread_running.store(true);
		g_receiver_thread = std::thread([app_id, disable_ice]()
									  {
    RunReceiverPluginWorker(app_id, disable_ice);
    g_receiver_thread_running.store(false); });
		return true;
	}

	UNITY_INTERFACE_EXPORT void SSPR_Stop()
	{
		std::lock_guard<std::mutex> lock(g_receiver_thread_mutex);
		g_receiver_stop_requested.store(true);
	...
	UNITY_INTERFACE_EXPORT bool SSPR_IsRunning()
	{
		return g_receiver_thread_running.load();
	}

	UNITY_INTERFACE_EXPORT void SSPR_SetPaused(bool paused)
	{
		g_receiver_pause_requested.store(paused);
	}

	UNITY_INTERFACE_EXPORT void SSPR_SetRemoteSteamIds(const std::uint64_t *steam_ids, int count)
	{
		...
		{
			std::lock_guard<std::mutex> lock(g_desired_senders_mutex);
			g_desired_senders = std::move(values);
		}
		g_desired_senders_dirty.store(true);
	}

	UNITY_INTERFACE_EXPORT void SSPR_ConfigureVideoLods(const SSPR_VideoLodConfig *lods, int count)
	{
		...
		{
			std::lock_guard<std::mutex> lock(g_receiver_video_lod_configs_mutex);
			g_receiver_video_lod_configs = std::move(values);
		}
		g_receiver_video_lod_configs_dirty.store(true);
	}

	UNITY_INTERFACE_EXPORT void SSPR_SetMaxAcceptedVideoLod(std::uint64_t sender_steam_id, std::int32_t lod)
	{
		...
		{
			std::lock_guard<std::mutex> lock(g_max_accepted_qualities_mutex);
			g_max_accepted_qualities[sender_steam_id].video_lod = NormalizeLod(lod);
		}
		g_max_accepted_qualities_dirty.store(true);
	}

	UNITY_INTERFACE_EXPORT void SSPR_SetMaxAcceptedAudioLod(std::uint64_t sender_steam_id, std::int32_t lod)
	{
		if (sender_steam_id == 0)
		{
			return;
		...
		{
			std::lock_guard<std::mutex> lock(g_max_accepted_qualities_mutex);
			g_max_accepted_qualities[sender_steam_id].audio_lod = NormalizeLod(lod);
		}
		g_max_accepted_qualities_dirty.store(true);
	}

	UNITY_INTERFACE_EXPORT void SSPR_SetOutputTextureForSteamId(std::uint64_t sender_steam_id, void *native_texture_ptr)
	{
		if (sender_steam_id == 0)
			return;
		std::lock_guard<std::mutex> lock(g_unity_mutex);
		...
		{
			g_unity_output_bindings.erase(sender_steam_id);
			return;
		}
		UnityOutputBinding &b = g_unity_output_bindings[sender_steam_id];
		b.target_texture = static_cast<ID3D11Texture2D *>(native_texture_ptr);
		b.opened_source_texture.Reset();
		b.opened_source_handle = nullptr;
	}

	UNITY_INTERFACE_EXPORT void SSPR_SetOutputTexture(void *native_texture_ptr)
	{
		std::uint64_t sender_id = FirstDesiredSender();
		if (sender_id == 0)
			return;
		SSPR_SetOutputTextureForSteamId(sender_id, native_texture_ptr);
	}

	UNITY_INTERFACE_EXPORT UnityRenderingEvent SSPR_GetRenderEventFunc()
	{
		return OnReceiverRenderEvent;
	}

	UNITY_INTERFACE_EXPORT int SSPR_GetRenderEventId()
	{
		return kReceiverRenderEventId;
	}

	UNITY_INTERFACE_EXPORT void SSPR_GetStats(SSPR_Stats *out_stats)
	{
		...
		out_stats->local_steam_id = g_receiver_local_steam_id.load();
		out_stats->connected = connected > 0 ? 1 : 0;
		out_stats->preview_width = preview_width;
		out_stats->preview_height = preview_height;
		out_stats->paused_by_sender = paused_by_sender > 0 ? 1 : 0;
	}

	UNITY_INTERFACE_EXPORT int SSPR_GetPeerStats(SSPR_PeerStats *out_stats, int max_count)
	{
		...
		for (int i = 0; i < n; ++i)
		{
			...
			out_stats[i].paused_by_sender = s.paused_by_sender;
	...
	UNITY_INTERFACE_EXPORT int SSPR_ReadAudioForSteamId(std::uint64_t sender_steam_id, float *out_samples, int max_samples)
	{
		if (sender_steam_id == 0 || out_samples == nullptr || max_samples <= 0)
		{
			return 0;
		}
		...
		{
			std::lock_guard<std::mutex> lock(g_peer_audio_mutex);
			auto it = g_peer_audio_rings.find(sender_steam_id);
	...
	UNITY_INTERFACE_EXPORT void SSPR_MarkAudioPushedForSteamId(std::uint64_t sender_steam_id)
	{
		if (sender_steam_id == 0)
		{
			return;
		...
		std::lock_guard<std::mutex> export_lock(g_peer_exports_mutex);
		auto export_it = g_peer_exports.find(sender_steam_id);
		...
		std::int64_t sender_now_us = static_cast<std::int64_t>(streamproto::NowUnixMicros());
		sender_now_us += static_cast<std::int64_t>(std::llround(export_it->second.clock_offset_ms * 1000.0));
		if (sender_now_us >= 0 && static_cast<std::uint64_t>(sender_now_us) >= export_it->second.last_audio_capture_ts_us)
		{
			export_it->second.last_audio_capture_to_unity_push_ms =
				static_cast<double>(static_cast<std::uint64_t>(sender_now_us) - export_it->second.last_audio_capture_ts_us) / 1000.0;
		}
	}

	UNITY_INTERFACE_EXPORT void SSPR_GetAudioFormat(int *out_sample_rate, int *out_channels)
	{
	...
	UNITY_INTERFACE_EXPORT const char *SSPR_GetLastError()
	{
		std::lock_guard<std::mutex> lock(g_error_mutex);
