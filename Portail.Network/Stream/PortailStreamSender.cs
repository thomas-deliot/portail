...
namespace Portail.Stream
{
	...
	public sealed class PortailStreamSender : MonoBehaviour
	{
		[Header("Lifecycle")]
		...
		public static PortailStreamSender Instance { get; private set; }
		...
		public void SetManualSenderTargets(IReadOnlyList<ulong> receiverSteamIds)
		{
			...
			PortailStreamNetworkUtility.SanitizeSteamIds(receiverSteamIds, manualSenderTargetSteamIds, _steamIdSeen);
		}
		...
		void PublishLocalSteamIdentity()
		{
			...
			ulong localSteamId = PortailStreamNetworkUtility.ResolveLocalSteamId(_localSource, _sender, null);
			if (localSteamId != 0)
		...
		void EnsureSenderStreamRunning()
		{
			...
			if (!PortailStreamNetworkUtility.EnsureSteamAvailable("PortailStreamSender", ref _steamUnavailableLogged))
			{
				StopSenderStream();
				return;
			}
		...
		void PullStats(float now)
		{
			if (_sender != null && _sender.IsRunning())
			{
				...
				PortailStreamNetworkUtility.UpdateRateSample(ref _senderRateSample, _senderStats.sent_video_bytes + _senderStats.sent_audio_bytes, frameCounter, now);
				PortailStreamNetworkUtility.UpdateRateSample(ref _senderEncodedVideoRateSample, _senderStats.encoded_video_bytes, frameCounter, now);
				PortailStreamNetworkUtility.UpdateRateSample(ref _senderEncodedAudioRateSample, _senderStats.encoded_audio_bytes, _senderStats.encoded_audio_frames, now);
				PullLodStats(now);
				PullPeerStats(now);
				return;
			}
			...
			PortailStreamNetworkUtility.UpdateRateSample(ref _senderRateSample, 0, 0, now);
			PortailStreamNetworkUtility.UpdateRateSample(ref _senderEncodedVideoRateSample, 0, 0, now);
			PortailStreamNetworkUtility.UpdateRateSample(ref _senderEncodedAudioRateSample, 0, 0, now);
		}
		...
		void PullLodStats(float now)
		{
			...
			for (int i = 0; i < _videoLodStats.Length; ++i)
			{
				...
				PortailStreamNetworkUtility.UpdateRateSample(ref sample, stats.encoded_video_bytes, stats.encoded_frames, now);
			...
			for (int i = 0; i < _audioLodStats.Length; ++i)
			{
				...
				PortailStreamNetworkUtility.UpdateRateSample(ref sample, stats.encoded_audio_bytes, stats.encoded_audio_frames, now);
		...
		void PullPeerStats(float now)
		{
			...
			for (int i = 0; i < peerStats.Length; ++i)
			{
				...
				PortailStreamNetworkUtility.UpdateRateSample(ref sample, stats.sent_video_bytes + stats.sent_audio_bytes, stats.encoded_frames, now);
		...
		List<ulong> BuildDesiredSenderTargets()
		{
			List<ulong> desiredTargets = autoTargetAllRemotePlayers
				? PortailStreamNetworkUtility.BuildRemoteStreamIds(_players, _desiredSenderTargetsScratch, _steamIdSeen, _localSource, requireBroadcasting: false)
				: PortailStreamNetworkUtility.SanitizeSteamIds(manualSenderTargetSteamIds, _desiredSenderTargetsScratch, _steamIdSeen);
		...
		void ApplySenderTargetsIfChanged(IReadOnlyList<ulong> desiredTargets)
		{
			...
			if (PortailStreamNetworkUtility.SteamIdListsEqual(_lastAppliedSenderTargets, desiredTargets))
				return;

			_sender.SetReceiverSteamIds(desiredTargets);
			PortailStreamNetworkUtility.CopySteamIds(desiredTargets, _lastAppliedSenderTargets);
		}
