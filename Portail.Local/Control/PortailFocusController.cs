...
namespace Portail.Control
{
	...
	public sealed class PortailFocusController : MonoBehaviour
	{
		...
		private bool IsValidDesktopScreenHit(RaycastHit hit)
		{
			...
			IPortailFeedPlayback streamPlayer = FindFeedPlayback(hit.collider);
		...
		private static IPortailFeedPlayback FindFeedPlayback(Collider collider)
		{
			if (collider == null)
			{
				return null;
			...
			{
				if (behaviours[i] is IPortailFeedPlayback playback)
				{
					return playback;
				}
			}
