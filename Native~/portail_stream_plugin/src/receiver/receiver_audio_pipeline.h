...
namespace streamproto::receiver
{
	...
	struct ReceiverAudioEvent
	{
		std::vector<std::uint8_t> bytes;
	};

	class ReceiverAudioPipeline
	{
	public:
		ReceiverAudioPipeline() = default;
		~ReceiverAudioPipeline() { Shutdown(); }

		ReceiverAudioPipeline(const ReceiverAudioPipeline &) = delete;
		ReceiverAudioPipeline &operator=(const ReceiverAudioPipeline &) = delete;
		...
		void AppendStats(ReceiverStats &stats) const
		{
	...
	private:
			...
			if (!decoded_audio.empty() && ring != nullptr)
			{
				...
				if (frame_header->capture_timestamp_us != 0)
				{
					...
					if (synced)
					{
						...
						sender_now_us += static_cast<std::int64_t>(std::llround(offset_us));
						if (sender_now_us >= 0 && static_cast<std::uint64_t>(sender_now_us) >= frame_header->capture_timestamp_us)
						{
							last_audio_capture_to_push_ms_.store(
								static_cast<double>(static_cast<std::uint64_t>(sender_now_us) - frame_header->capture_timestamp_us) / 1000.0,
		...
		std::deque<ReceiverAudioEvent> audio_events_;
