...
namespace streamproto::host
{
	...
	class HostPeerSender
	{
	public:
		HostPeerSender(SteamSdr &sdr, std::uint64_t steam_id)
		...
		HostPeerSender(const HostPeerSender &) = delete;
		HostPeerSender &operator=(const HostPeerSender &) = delete;
		...
		HostSendEnqueueResult Enqueue(std::vector<std::uint8_t> bytes, bool reliable, SdrLane lane)
		{
			HostSendEnqueueResult result{};
			if (bytes.empty())
			{
				return result;
			}
	...
	using HostPeerSenderMap = std::unordered_map<std::uint64_t, std::shared_ptr<HostPeerSender>>;
