...
namespace streamproto::host
{
	...
	struct HostMediaTarget
	{
		std::uint64_t steam_id = 0;
		std::uint32_t generation = 0;
	};

	struct HostPeerSendDelta
	{
	...
	struct HostMediaSendResult
	{
		...
		std::uint64_t sent_audio_frames = 0;
		std::uint64_t sent_audio_bytes = 0;
		std::unordered_map<std::uint64_t, HostPeerSendDelta> peer_deltas;
	};

	inline HostMediaSendResult SendAudioFrame(
		const HostPeerSenderMap &peer_senders,
		const std::vector<HostMediaTarget> &targets,
		...
		int lod_index)
	{
		...
		for (const HostMediaTarget &target : targets)
		{
			...
			HostSendEnqueueResult enqueue = sender_it->second->Enqueue(std::move(packet), false, streamproto::SdrLane::kAudio);
			...
			if (enqueue.queued)
			{
				...
				HostPeerSendDelta &delta = result.peer_deltas[target.steam_id];
	...
	inline HostMediaSendResult SendEncodedPacket(
		const HostPeerSenderMap &peer_senders,
		const std::vector<HostMediaTarget> &targets,
		...
		int lod_index)
	{
		...
		for (std::size_t i = 0; i < data_chunks; ++i)
		{
			...
			for (const HostMediaTarget &target : targets)
			{
				...
				HostSendEnqueueResult enqueue = sender_it->second->Enqueue(std::move(queued_msg), send_reliable_for_frame, streamproto::SdrLane::kVideo);
				if (enqueue.dropped_old || enqueue.dropped_new)
				{
					result.queue_drop = true;
				}
