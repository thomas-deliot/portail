...
namespace streamproto::host
{
	...
	struct HostPeerStreamState
	{
		std::int32_t max_video_lod = 0;
		std::int32_t max_audio_lod = 0;
		HostPeerLodAssignment assigned_lod{};
		...
		bool host_pause_state_sent = false;
		bool last_host_pause_sent = false;
	...
	{
		HostVideoLodConfigState config{};
		HostOptions options{};
	...
	{
		HostAudioLodConfigState config{};
	...
	inline void BeginVideoGenerationTransition(HostPeerStreamState &state, std::uint64_t now_ms)
	{
		state.target_video_generation = NextStreamGeneration(state.target_video_generation);
		state.client_applied_video_generation = 0;
	...
	inline void BeginAudioGenerationTransition(HostPeerStreamState &state, std::uint64_t now_ms)
	{
		state.target_audio_generation = NextStreamGeneration(state.target_audio_generation);
		state.client_applied_audio_generation = 0;
	...
	inline bool HasPendingVideoGeneration(const HostPeerStreamState &state)
	{
		return state.media_video_generation != state.target_video_generation;
	}

	inline bool HasPendingAudioGeneration(const HostPeerStreamState &state)
	{
