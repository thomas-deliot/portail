...
namespace streamproto::host
{
	...
	public:
		bool Ensure(std::string *out_error = nullptr)
		{
			...
			if (module == nullptr)
			{
				std::string error = "Portail Stream host requires portail_capture_plugin.dll to be loaded beside the Portail Stream native plugins: " + Win32ErrorString(GetLastError());
