...
namespace streamproto::client
{
	...
	struct ClientMaxAcceptedQuality
	{
	...
	inline bool SendPing(SteamSdr &sdr, std::uint32_t sequence, std::uint64_t client_send_ts_us)
	{
		proto::PingMessage msg{};
		proto::InitMessageHeader(msg, proto::MessageType::kPing);
		msg.sequence = sequence;
		msg.reserved = 0;
		msg.client_send_timestamp_us = client_send_ts_us;
		return sdr.Send(&msg, sizeof(msg), true);
	}
	...
	inline bool SendClientMaxAcceptedQuality(SteamSdr &sdr, const ClientMaxAcceptedQuality &quality, std::uint64_t sender_steam_id)
	{
		proto::StreamQualityMessage msg{};
		proto::InitMessageHeader(msg, proto::MessageType::kStreamQuality);
		msg.command = static_cast<std::uint8_t>(proto::StreamQualityCommand::kClientMaxAccepted);
	...
	inline bool SendClientAppliedQuality(
		SteamSdr &sdr,
		std::uint64_t sender_steam_id,
		...
		std::uint32_t audio_generation)
	{
		proto::StreamQualityMessage msg{};
		proto::InitMessageHeader(msg, proto::MessageType::kStreamQuality);
		msg.command = static_cast<std::uint8_t>(proto::StreamQualityCommand::kClientApplied);
