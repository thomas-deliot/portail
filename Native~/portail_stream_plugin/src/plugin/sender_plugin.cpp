...
#include "sender/sender_media_sender.h"
#include "sender/sender_peer_sender.h"
#include "sender/sender_options.h"
#include "sender/sender_route_state.h"
#include "sender/sender_runtime_state.h"
#include "sender/sender_stats.h"
#include "sender/sdr_queue_status.h"
#include "sender/shared_texture_mirror.h"
#include "sender/virtual_desktop_capture_bridge.h"
#include "plugin/sender_plugin_api.h"
...
namespace
{
	...
	using streamproto::sender::EncodedPacket;
	using streamproto::sender::DefaultAudioLodConfigs;
	using streamproto::sender::DefaultVideoLodConfigs;
	using streamproto::sender::FfmpegEncoder;
	using streamproto::sender::HasKeyframePacket;
	using streamproto::sender::SenderAudioLodConfigState;
	using streamproto::sender::SenderAudioLodExportState;
	using streamproto::sender::SenderMediaSendResult;
	using streamproto::sender::SenderMediaTarget;
	using streamproto::sender::SenderPeerStreamState;
	using streamproto::sender::SenderPeerExportState;
	using streamproto::sender::SenderPeerLodAssignment;
	using streamproto::sender::SenderPeerSendDelta;
	using streamproto::sender::SenderPeerSender;
	using streamproto::sender::SenderPeerSenderMap;
	using streamproto::sender::SenderRouteSnapshot;
	using streamproto::sender::SenderOptions;
	using streamproto::sender::BuildSenderStatsLine;
	using streamproto::sender::SenderStats;
	using streamproto::sender::SenderVideoLodConfigState;
	using streamproto::sender::SenderVideoLodExportState;
	using streamproto::sender::VideoLodRuntime;
	using streamproto::sender::AudioLodRuntime;
	using streamproto::sender::VideoLodCanProduce;
	using streamproto::sender::VideoLodCanWarmEncoder;
	using streamproto::sender::AudioLodCanProduce;
	using streamproto::sender::BeginVideoGenerationTransition;
	using streamproto::sender::BeginAudioGenerationTransition;
	using streamproto::sender::HasPendingVideoGeneration;
	using streamproto::sender::HasPendingAudioGeneration;
	using streamproto::sender::kMaxConfiguredLods;
	using streamproto::sender::NormalizeVideoRateControl;
	using streamproto::sender::RawVideoFrame;
	using streamproto::sender::SanitizeAudioLodConfig;
	using streamproto::sender::SanitizeVideoLodConfig;
	using streamproto::sender::SendAudioFrame;
	using streamproto::sender::SendEncodedPacket;
	using streamproto::sender::SetFfmpegEncoderLogCallbacks;
	using streamproto::sender::SharedRawVideoFrame;
	using streamproto::sender::SenderVideoFrameHub;
	using streamproto::sender::SenderAudioFrameHub;
	using streamproto::sender::SharedTextureMirror;
	using streamproto::sender::SdrQueueStatus;
	using streamproto::sender::ApplyConnectionSendRateLimit;
	using streamproto::sender::ComputeSendRateBytesPerSecForBitrate;
	using streamproto::sender::ComputeUnreliableChunkPayloadLimit;
	using streamproto::sender::GetSdrQueueStatusForSteamId;
	using streamproto::sender::ShouldRealtimeDrop;
	using streamproto::sender::VirtualDesktopCaptureBridge;
	using streamproto::sender::VirtualDesktopCaptureStats;
	using streamproto::sender::VideoRateControl;
	...
	constexpr int kSenderRenderEventId = 0x1001;

	std::atomic<bool> g_pause_stream{false};
	std::mutex g_target_receivers_mutex;
	std::vector<std::uint64_t> g_target_receivers;
	std::atomic<bool> g_target_receivers_dirty{false};
	...
	std::unordered_map<std::uint64_t, SenderPeerLodAssignment> g_assigned_peer_lods;
	std::atomic<bool> g_assigned_peer_lods_dirty{false};
	std::mutex g_lod_config_mutex;
	std::vector<SenderVideoLodConfigState> g_video_lod_configs;
	std::vector<SenderAudioLodConfigState> g_audio_lod_configs;
	...
	std::mutex g_sender_peer_exports_mutex;
	std::vector<SenderPeerExportState> g_sender_peer_exports;
	std::mutex g_sender_lod_exports_mutex;
	std::vector<SenderVideoLodExportState> g_sender_video_lod_exports;
	std::vector<SenderAudioLodExportState> g_sender_audio_lod_exports;
	std::atomic<bool> g_stats_connected{false};
	std::atomic<int> g_stats_connected_ice_receivers{0};
	std::atomic<int> g_stats_connected_sdr_receivers{0};
	...
	using SenderLogCallback = streamproto::log::Callback;
	std::mutex g_log_callback_mutex;
	SenderLogCallback g_log_callback = nullptr;
	std::atomic<bool> g_sender_logging_enabled{true};
	...
	std::vector<std::uint64_t> CopyTargetReceivers()
	{
		std::lock_guard<std::mutex> lock(g_target_receivers_mutex);
		return g_target_receivers;
	}

	std::vector<SenderVideoLodConfigState> CopyVideoLodConfigs()
	{
	...
	std::vector<SenderAudioLodConfigState> CopyAudioLodConfigs()
	{
		std::lock_guard<std::mutex> lock(g_lod_config_mutex);
	...
	void SetSenderLogCallback(SenderLogCallback callback)
	{
		std::lock_guard<std::mutex> lock(g_log_callback_mutex);
		g_log_callback = callback;
	}

	SenderLogCallback GetSenderLogCallback()
	{
		std::lock_guard<std::mutex> lock(g_log_callback_mutex);
		return g_log_callback;
	}

	void SetSenderLoggingEnabled(bool enabled)
	{
		g_sender_logging_enabled.store(enabled);
	}

	streamproto::log::Stream LogInfoStream()
	{
		return streamproto::log::Stream(std::cout, streamproto::log::Level::kInfo, GetSenderLogCallback(), g_sender_logging_enabled.load());
	}

	streamproto::log::Stream LogWarningStream()
	{
		return streamproto::log::Stream(std::cerr, streamproto::log::Level::kWarning, GetSenderLogCallback(), g_sender_logging_enabled.load());
	}

	streamproto::log::Stream LogErrorStream()
	{
		return streamproto::log::Stream(std::cerr, streamproto::log::Level::kError, GetSenderLogCallback(), g_sender_logging_enabled.load());
	}

	void LogSenderEncoderInfo(const std::string &message)
	{
		LogInfoStream() << message;
	}

	void LogSenderEncoderWarning(const std::string &message)
	{
	...
	void ApplyMediaSendResult(
		const SenderMediaSendResult &result,
		SenderStats &stats,
		std::unordered_map<std::uint64_t, SenderPeerStreamState> &stream_states,
		double send_ms,
		bool audio_lane)
	{
		...
		{
			SenderPeerStreamState &state = stream_states[steam_id];
	...
	SenderPeerSenderMap SnapshotPeerSenders(const SenderPeerSenderMap &peer_senders)
	{
		return peer_senders;
	}

	bool EnqueueReliableControl(
		SenderPeerSenderMap &peer_senders,
	...
	SenderOptions MakeVideoLodOptions(const SenderOptions &base, const SenderVideoLodConfigState &config)
	{
		SenderOptions options = base;
	...
	bool SendStreamQualityMessage(
		...
		const SenderPeerStreamState &state,
		...
		std::uint64_t sender_steam_id)
	{
		...
		return EnqueueReliableControl(peer_senders, receiver_steam_id, &msg, sizeof(msg));
	}

	std::unordered_map<std::uint64_t, SenderPeerLodAssignment> CopyAssignedPeerLods()
	{
		std::lock_guard<std::mutex> lock(g_assigned_peer_lods_mutex);
		return g_assigned_peer_lods;
	}

	void PublishSenderPeerStats(
		const SteamSdr &sdr,
		const SenderStats &stats,
		const std::unordered_map<std::uint64_t, SenderPeerStreamState> &stream_states,
		std::uint64_t available_video_lod_mask,
		std::uint64_t available_audio_lod_mask)
	{
		std::vector<SenderPeerExportState> exports;
		const std::vector<streamproto::SenderPeerStatus> peers = sdr.SenderPeerStatuses();
		exports.reserve(peers.size());
		for (const streamproto::SenderPeerStatus &peer : peers)
		{
			if (peer.steam_id == 0)
			{
				continue;
			...
			SenderPeerExportState export_state{};
			export_state.receiver_steam_id = peer.steam_id;
			...
			export_state.paused_by_receiver = peer.paused ? 1 : 0;
			auto state_it = stream_states.find(peer.steam_id);
			if (state_it != stream_states.end())
			{
				const SenderPeerStreamState &state = state_it->second;
		...
		std::lock_guard<std::mutex> lock(g_sender_peer_exports_mutex);
		g_sender_peer_exports.swap(exports);
	}

	void PublishSenderLodStats(
		const std::vector<std::unique_ptr<VideoLodRuntime>> &video_lods,
		const std::vector<std::unique_ptr<AudioLodRuntime>> &audio_lods)
	{
		std::vector<SenderVideoLodExportState> video_exports;
		video_exports.reserve(video_lods.size());
		for (std::size_t i = 0; i < video_lods.size(); ++i)
		{
			const VideoLodRuntime &lod = *video_lods[i];
			SenderVideoLodExportState export_state{};
			...
			export_state.receiver_count = lod.receiver_count;
		...
		std::vector<SenderAudioLodExportState> audio_exports;
		audio_exports.reserve(audio_lods.size());
		for (std::size_t i = 0; i < audio_lods.size(); ++i)
		{
			const AudioLodRuntime &lod = *audio_lods[i];
			SenderAudioLodExportState export_state{};
			export_state.index = static_cast<std::int32_t>(i);
			export_state.enabled = lod.config.enabled ? 1 : 0;
		...
		std::lock_guard<std::mutex> lock(g_sender_lod_exports_mutex);
		g_sender_video_lod_exports.swap(video_exports);
		g_sender_audio_lod_exports.swap(audio_exports);
	}
	...
	bool SendHello(SenderPeerSenderMap &peer_senders, std::uint32_t app_id, std::uint64_t steam_id, std::uint64_t target_steam_id = 0)
	{
	...
	bool SendPong(SenderPeerSenderMap &peer_senders, std::uint64_t target_steam_id, const proto::PingMessage &ping, std::uint64_t sender_recv_ts_us)
	{
		proto::PongMessage msg{};
		proto::InitMessageHeader(msg, proto::MessageType::kPong);
		msg.sequence = ping.sequence;
		msg.reserved = 0;
		msg.receiver_send_timestamp_us = ping.receiver_send_timestamp_us;
		msg.sender_recv_timestamp_us = sender_recv_ts_us;
		msg.sender_send_timestamp_us = streamproto::NowUnixMicros();
		return EnqueueReliableControl(peer_senders, target_steam_id, &msg, sizeof(msg));
	}
	...
	constexpr std::uint64_t kSenderStatsExportIntervalMs = 50;

	void PublishSenderStatsSnapshot(const SenderStats &stats, bool preserve_capture_timing)
	{
		std::lock_guard<std::mutex> lock(g_stats_mutex);
	...
	void PrintStatsLine(
		...
		SenderStats &last_snapshot)
	{
		LogInfoStream() << '\r' << BuildSenderStatsLine(stats, encoder_name, path_name, connected, delta_ms, last_snapshot) << std::flush;
	...
	bool HasAnyTargets(const std::vector<std::vector<SenderMediaTarget>> &targets_by_lod)
	{
	...
	int RunSenderWithOptions(SenderOptions options)
	{
		SetFfmpegEncoderLogCallbacks(&LogSenderEncoderInfo, &LogSenderEncoderWarning);
		...
		{
			// COM apartment may already be initialized by sender process.
		}

		std::string error;

		std::vector<SenderVideoLodConfigState> configured_video_lods = CopyVideoLodConfigs();
		std::vector<SenderAudioLodConfigState> configured_audio_lods = CopyAudioLodConfigs();
		...
		for (const SenderVideoLodConfigState &config : configured_video_lods)
		{
		...
		for (const SenderAudioLodConfigState &config : configured_audio_lods)
		{
		...
		sdr_config.role = SdrRole::kSender;
		...
		LogInfoStream() << "path=sdr Waiting for receiver connection...";
		if (g_target_receivers_dirty.exchange(false))
		{
			sdr.SetSenderAllowedSteamIds(CopyTargetReceivers());
		}
		...
		std::unordered_map<std::uint64_t, SenderPeerStreamState> peer_stream_states;
		SenderPeerSenderMap peer_senders;
		std::mutex runtime_mutex;
		std::mutex peer_senders_mutex;
		std::mutex route_mutex;
		std::condition_variable route_cv;
		SenderRouteSnapshot route_snapshot{};
		...
		SenderVideoFrameHub video_frame_hub{};
		SenderAudioFrameHub audio_frame_hub{};
		...
		std::unordered_map<std::uint64_t, SenderPeerLodAssignment> assigned_peer_lods = CopyAssignedPeerLods();
		...
		auto apply_route_profile = [&](ConnectionPath path, bool force_config_send)
		{
			...
			if (force_config_send)
			{
				...
				for (auto &[receiver_id, peer_state] : peer_stream_states)
				{
					(void)receiver_id;
		...
		auto copy_route_snapshot = [&]() -> SenderRouteSnapshot
		{
			std::lock_guard<std::mutex> lock(route_mutex);
			return route_snapshot;
		};

		auto publish_route_snapshot = [&](SenderRouteSnapshot snapshot)
		{
			{
				std::lock_guard<std::mutex> lock(route_mutex);
		...
		auto video_capture_loop = [&]()
		{
			...
			{
				const SenderRouteSnapshot route = copy_route_snapshot();
		...
		auto video_lod_loop = [&](std::size_t lod_index)
		{
			...
			while (g_running.load() && !media_workers_stop.load(std::memory_order_acquire))
			{
				...
				SenderRouteSnapshot route = copy_route_snapshot();
				...
				if (encoder_ready && lod_bit != 0 && route.video_source_available && has_pending_config_targets)
				{
					...
					for (std::uint64_t target : route.connected_peer_ids)
					{
						SenderPeerStreamState &state = peer_stream_states[target];
						if (state.effective_video_lod != static_cast<std::int32_t>(lod_index))
						{
							continue;
						}
				...
				std::vector<SenderMediaTarget> sendable_video_targets;
				sendable_video_targets.reserve(video_targets.size());
				int peer_realtime_drops = 0;
				for (const SenderMediaTarget &target : video_targets)
				{
				...
				SenderPeerSenderMap peer_sender_snapshot;
				{
					std::lock_guard<std::mutex> sender_lock(peer_senders_mutex);
				...
				for (const EncodedPacket &packet : packets)
				{
					...
					SenderMediaSendResult send_result = SendEncodedPacket(
				...
				const auto send_end = std::chrono::steady_clock::now();
				{
					...
					if (!frame_send_error)
					{
						for (const SenderMediaTarget &target : sendable_video_targets)
						{
		...
		auto audio_lod_loop = [&](std::size_t lod_index)
		{
			while (g_running.load() && !media_workers_stop.load(std::memory_order_acquire))
			{
				...
				const std::vector<SenderMediaTarget> audio_targets =
				...
				SenderMediaSendResult audio_result = SendAudioFrame(
					peer_sender_snapshot,
					audio_targets,
					audio_frame,
					audio_packet,
		...
		while (g_running.load())
		{
			...
			const std::vector<streamproto::SenderPeerStatus> peer_statuses = sdr.SenderPeerStatuses();
			g_stats_connected.store(!peer_statuses.empty());
			int connected_ice_receivers = 0;
			int connected_sdr_receivers = 0;
			std::unordered_set<std::uint64_t> connected_peer_ids;
			connected_peer_ids.reserve(peer_statuses.size());
			for (const streamproto::SenderPeerStatus &peer_status : peer_statuses)
			{
				...
				{
					++connected_sdr_receivers;
				}
				else
				{
					++connected_ice_receivers;
				}
			}
			g_stats_connected_ice_receivers.store(connected_ice_receivers);
			g_stats_connected_sdr_receivers.store(connected_sdr_receivers);
			...
			std::unordered_map<std::uint64_t, SenderPeerLodAssignment> assigned_peer_lods_update;
			...
			std::vector<SenderVideoLodConfigState> video_configs;
			std::vector<SenderAudioLodConfigState> audio_configs;
			...
			// prevents connect/disconnect churn from blocking the media LOD workers.
			{
				...
				for (std::uint64_t receiver_id : connected_peer_ids)
				{
					if (peer_senders.find(receiver_id) == peer_senders.end())
					{
						auto sender = std::make_shared<SenderPeerSender>(sdr, receiver_id);
						sender->Start();
						peer_senders.emplace(receiver_id, std::move(sender));
					}
				}
			}

			// Steam receive/message parsing is intentionally outside the sender supervisor's
			...
			sdr.ReceiveWithPeer([&](std::uint64_t remote_steam_id, const std::uint8_t *data, std::size_t bytes)
				...
				if (type == proto::MessageType::kPing)
				{
					...
					const std::uint64_t sender_recv_ts_us = streamproto::NowUnixMicros();
					{
						std::lock_guard<std::mutex> sender_lock(peer_senders_mutex);
						SendPong(peer_senders, remote_steam_id, *ping, sender_recv_ts_us);
					}
					return;
				}
				...
				if (type == proto::MessageType::kStreamQuality)
				{
					...
					std::lock_guard<std::mutex> runtime_lock(runtime_mutex);
					if (command == proto::StreamQualityCommand::kReceiverMaxAccepted)
					{
						SenderPeerStreamState &state = peer_stream_states[remote_steam_id];
					...
					else if (command == proto::StreamQualityCommand::kReceiverApplied)
					{
						SenderPeerStreamState &state = peer_stream_states[remote_steam_id];
						...
						{
							state.receiver_applied_video_generation = quality_msg->sequence;
						}
						...
						{
							state.receiver_applied_audio_generation = quality_msg->profile_id;
						}
					}
					return;
				}
				...
				if (type == proto::MessageType::kStreamControl)
				{
					...
					if (command == proto::StreamControlCommand::kReceiverPause)
					{
						sdr.SetSenderPeerPaused(sender, true);
					}
					else if (command == proto::StreamControlCommand::kReceiverResume)
					{
						sdr.SetSenderPeerPaused(sender, false);
					}
				}
			});
			...
			std::vector<std::vector<SenderMediaTarget>> video_targets_by_lod(video_lods.size());
			std::vector<std::vector<SenderMediaTarget>> audio_targets_by_lod(audio_lods.size());
			{
				std::lock_guard<std::mutex> runtime_lock(runtime_mutex);
				...
				for (std::uint64_t receiver_id : connected_peer_ids)
				{
					SenderPeerStreamState &state = peer_stream_states[receiver_id];
					auto assigned_it = assigned_peer_lods.find(receiver_id);
					state.assigned_lod = assigned_it != assigned_peer_lods.end() ? assigned_it->second : SenderPeerLodAssignment{};
				}
				...
				for (const streamproto::SenderPeerStatus &peer_status : peer_statuses)
				{
					...
					SenderPeerStreamState &state = peer_stream_states[peer_status.steam_id];
					...
					if (HasPendingVideoGeneration(state))
					{
						...
						const bool receiver_ready = state.receiver_applied_video_generation == state.target_video_generation;
						...
						if (receiver_ready || grace_expired || no_media_to_keep || old_media_unavailable)
						{
					...
					if (HasPendingAudioGeneration(state))
					{
						...
						const bool receiver_ready = state.receiver_applied_audio_generation == state.target_audio_generation;
						const bool grace_expired =
					...
					if (!state.hello_sent)
					{
						...
						LogInfoStream() << "path=" << (peer_status.relayed ? "sdr" : "ice")
										<< " Receiver connected: " << peer_status.steam_id;
					}

					const bool awaiting_generation_ack =
						state.receiver_applied_video_generation != state.target_video_generation ||
						state.receiver_applied_audio_generation != state.target_audio_generation;
						...
						quality_resend_due)
					{
						...
						if (SendStreamQualityMessage(
								...
								proto::StreamQualityCommand::kSenderAssigned,
								state,
								available_video_lod_mask,
								available_audio_lod_mask,
								sdr.LocalSteamId()))
						...
						static_cast<std::size_t>(state.media_video_lod) < video_targets_by_lod.size())
					{
						video_targets_by_lod[static_cast<std::size_t>(state.media_video_lod)].push_back(
							SenderMediaTarget{peer_status.steam_id, state.media_video_generation});
					}
					if (!peer_status.paused &&
						...
						static_cast<std::size_t>(state.media_audio_lod) < audio_targets_by_lod.size())
					{
						audio_targets_by_lod[static_cast<std::size_t>(state.media_audio_lod)].push_back(
							SenderMediaTarget{peer_status.steam_id, state.media_audio_generation});
					}
				}

				for (std::size_t i = 0; i < video_lods.size(); ++i)
				{
					video_lods[i]->receiver_count = static_cast<int>(video_targets_by_lod[i].size());
				}
				for (std::size_t i = 0; i < audio_lods.size(); ++i)
				{
					audio_lods[i]->receiver_count = static_cast<int>(audio_targets_by_lod[i].size());
				}
			...
			const bool pause_now = g_pause_stream.load();
			{
				...
				{
					SenderPeerStreamState &state = peer_stream_states[peer_status.steam_id];
					if (!state.sender_pause_state_sent || state.last_sender_pause_sent != pause_now)
					{
						if (SendStreamControl(
								peer_senders,
								pause_now ? proto::StreamControlCommand::kSenderPause : proto::StreamControlCommand::kSenderResume,
								sdr.LocalSteamId(),
								peer_status.steam_id))
						{
							state.sender_pause_state_sent = true;
							state.last_sender_pause_sent = pause_now;
						}
					}
				}
			}
...

			{
				...
				SenderRouteSnapshot next_route{};
			...
			if (publish_now_ms - last_stats_export_ms >= kSenderStatsExportIntervalMs)
			{
				PullVirtualDesktopCaptureStats();
				SenderStats stats_export{};
				std::unordered_map<std::uint64_t, SenderPeerStreamState> peer_states_export;
				...
				PublishSenderStatsSnapshot(stats_export, true);
				PublishSenderPeerStats(sdr, stats_export, peer_states_export, video_lod_mask_export, audio_lod_mask_export);
				{
					std::lock_guard<std::mutex> runtime_lock(runtime_mutex);
					PublishSenderLodStats(video_lods, audio_lods);
				}
		...
		LogInfoStream() << "path=" << ConnectionPathName(active_path) << " Stopping sender sender.";
		sdr.Close("sender-exit");
		ClearVirtualDesktopNetworkAudio();
		g_stats_connected.store(false);
		g_stats_connected_ice_receivers.store(0);
		g_stats_connected_sdr_receivers.store(0);
		{
			std::lock_guard<std::mutex> lock(g_sender_peer_exports_mutex);
			g_sender_peer_exports.clear();
		}
		{
			std::lock_guard<std::mutex> lock(g_sender_lod_exports_mutex);
			g_sender_video_lod_exports.clear();
			g_sender_audio_lod_exports.clear();
		}
		return 0;
	}
...
namespace
{
	...
	std::mutex g_sender_thread_mutex;
	std::atomic<bool> g_sender_thread_running{false};


	void UNITY_INTERFACE_API OnSenderGraphicsDeviceEvent(UnityGfxDeviceEventType event_type)
	{
	...
	void UNITY_INTERFACE_API OnSenderRenderEvent(int event_id)
	{
		if (event_id != kSenderRenderEventId)
		{
			return;
		}
...
extern "C"
{
	...
	UNITY_INTERFACE_EXPORT void UNITY_INTERFACE_API UnityPluginLoad(IUnityInterfaces *unity_interfaces)
	{
		...
		{
			g_unity_graphics->RegisterDeviceEventCallback(OnSenderGraphicsDeviceEvent);
		}
		OnSenderGraphicsDeviceEvent(kUnityGfxDeviceEventInitialize);
	}
	...
	UNITY_INTERFACE_EXPORT void UNITY_INTERFACE_API UnityPluginUnload()
	{
		if (g_unity_graphics != nullptr)
		{
			g_unity_graphics->UnregisterDeviceEventCallback(OnSenderGraphicsDeviceEvent);
		}
		OnSenderGraphicsDeviceEvent(kUnityGfxDeviceEventShutdown);
		SetSenderLogCallback(nullptr);
		SetSenderLoggingEnabled(true);
		g_unity_interfaces = nullptr;
		g_unity_graphics = nullptr;
	}

	UNITY_INTERFACE_EXPORT void SSPS_ConfigureVideoLods(const SSPS_VideoLodConfig *lods, int count)
	{
		std::vector<SenderVideoLodConfigState> values;
	...
	UNITY_INTERFACE_EXPORT void SSPS_ConfigureAudioLods(const SSPS_AudioLodConfig *lods, int count)
	{
		std::vector<SenderAudioLodConfigState> values;
		if (lods != nullptr && count > 0)
		{
			const int safe_count = std::clamp(count, 0, kMaxConfiguredLods);
			values.reserve(static_cast<std::size_t>(safe_count));
	...
	UNITY_INTERFACE_EXPORT void SSPS_SetVideoLodEnabled(int lod_index, bool enabled)
	{
		if (lod_index < 0)
			return;
		{
	...
	UNITY_INTERFACE_EXPORT void SSPS_SetAudioLodEnabled(int lod_index, bool enabled)
	{
		if (lod_index < 0)
			return;
		{
	...
	UNITY_INTERFACE_EXPORT bool SSPS_Start(const SSPS_StartParams *params)
	{
		...
		std::lock_guard<std::mutex> lock(g_sender_thread_mutex);
		if (g_sender_thread_running.load())
		{
			return false;
		}
		...
		{
			std::lock_guard<std::mutex> peer_lock(g_sender_peer_exports_mutex);
			g_sender_peer_exports.clear();
		}
		{
			std::lock_guard<std::mutex> lod_lock(g_sender_lod_exports_mutex);
			g_sender_video_lod_exports.clear();
			g_sender_audio_lod_exports.clear();
		}
		{
		...
		options.encoder_queue_depth = ReadIntEnvironment("SSPS_ENCODER_QUEUE_DEPTH", 1, 1, 8);
		...
		g_sender_thread_running.store(true);
		g_sender_thread = std::thread([options]()
									{
    const int result = RunSenderWithOptions(options);
    if (result != 0) {
      ...
      if (!has_error) {
        SetLastErrorString("Sender worker exited with failure.");
      }
    }
    g_sender_thread_running.store(false); });
		return true;
	}

	UNITY_INTERFACE_EXPORT void SSPS_Stop()
	{
		std::lock_guard<std::mutex> lock(g_sender_thread_mutex);
	...
	UNITY_INTERFACE_EXPORT bool SSPS_IsRunning()
	{
		return g_sender_thread_running.load();
	}

	UNITY_INTERFACE_EXPORT void SSPS_SetPaused(bool paused)
	{
		g_pause_stream.store(paused);
	}

	UNITY_INTERFACE_EXPORT void SSPS_SetReceiverSteamIds(const std::uint64_t *steam_ids, int count)
	{
		...
		{
			std::lock_guard<std::mutex> lock(g_target_receivers_mutex);
			g_target_receivers = std::move(values);
		}
		g_target_receivers_dirty.store(true);
	}

	UNITY_INTERFACE_EXPORT void SSPS_SetReceiverVideoLod(std::uint64_t receiver_steam_id, std::int32_t lod)
	{
		if (receiver_steam_id == 0)
		{
			return;
		}
		...
		{
			std::lock_guard<std::mutex> lock(g_assigned_peer_lods_mutex);
			g_assigned_peer_lods[receiver_steam_id].video_lod = NormalizeLod(lod);
		}
		g_assigned_peer_lods_dirty.store(true);
	}

	UNITY_INTERFACE_EXPORT void SSPS_SetReceiverAudioLod(std::uint64_t receiver_steam_id, std::int32_t lod)
	{
		if (receiver_steam_id == 0)
		{
			return;
		...
		{
			std::lock_guard<std::mutex> lock(g_assigned_peer_lods_mutex);
			g_assigned_peer_lods[receiver_steam_id].audio_lod = NormalizeLod(lod);
		}
		g_assigned_peer_lods_dirty.store(true);
	}

	UNITY_INTERFACE_EXPORT void SSPS_SetPreviewTexture(void *native_texture_ptr)
	{
		std::lock_guard<std::mutex> lock(g_unity_mutex);
		g_unity_target_texture = static_cast<ID3D11Texture2D *>(native_texture_ptr);
	}

	UNITY_INTERFACE_EXPORT void SSPS_SetLogCallback(SenderLogCallback callback)
	{
		SetSenderLogCallback(callback);
	}

	UNITY_INTERFACE_EXPORT void SSPS_SetLoggingEnabled(bool enabled)
	{
		SetSenderLoggingEnabled(enabled);
	}

	UNITY_INTERFACE_EXPORT UnityRenderingEvent SSPS_GetRenderEventFunc()
	{
		return OnSenderRenderEvent;
	}

	UNITY_INTERFACE_EXPORT int SSPS_GetRenderEventId()
	{
		return kSenderRenderEventId;
	}

	UNITY_INTERFACE_EXPORT void SSPS_GetStats(SSPS_Stats *out_stats)
	{
		...
		out_stats->connected_ice_receivers = g_stats_connected_ice_receivers.load();
		out_stats->connected_sdr_receivers = g_stats_connected_sdr_receivers.load();
	...
	UNITY_INTERFACE_EXPORT int SSPS_GetPeerStats(SSPS_PeerStats *out_stats, int max_count)
	{
		...
		std::vector<SenderPeerExportState> snapshot;
		{
			std::lock_guard<std::mutex> lock(g_sender_peer_exports_mutex);
			snapshot = g_sender_peer_exports;
		}

		std::sort(snapshot.begin(), snapshot.end(), [](const SenderPeerExportState &a, const SenderPeerExportState &b)
				  { return a.receiver_steam_id < b.receiver_steam_id; });
		const int count = std::min<int>(max_count, static_cast<int>(snapshot.size()));
		for (int i = 0; i < count; ++i)
		{
			const SenderPeerExportState &source = snapshot[static_cast<std::size_t>(i)];
			out_stats[i].receiver_steam_id = source.receiver_steam_id;
			...
			out_stats[i].paused_by_receiver = source.paused_by_receiver;
	...
	UNITY_INTERFACE_EXPORT int SSPS_GetVideoLodStats(SSPS_VideoLodStats *out_stats, int max_count)
	{
		if (out_stats == nullptr || max_count <= 0)
		{
			return 0;
		}
		std::vector<SenderVideoLodExportState> snapshot;
		{
			std::lock_guard<std::mutex> lock(g_sender_lod_exports_mutex);
			snapshot = g_sender_video_lod_exports;
		}
		const int count = std::min<int>(max_count, static_cast<int>(snapshot.size()));
		for (int i = 0; i < count; ++i)
		{
			const SenderVideoLodExportState &source = snapshot[static_cast<std::size_t>(i)];
			...
			out_stats[i].receiver_count = source.receiver_count;
			out_stats[i].encoded_frames = source.encoded_frames;
			out_stats[i].encoded_video_bytes = source.encoded_video_bytes;
	...
	UNITY_INTERFACE_EXPORT int SSPS_GetAudioLodStats(SSPS_AudioLodStats *out_stats, int max_count)
	{
		if (out_stats == nullptr || max_count <= 0)
		{
			return 0;
		}
		std::vector<SenderAudioLodExportState> snapshot;
		{
			std::lock_guard<std::mutex> lock(g_sender_lod_exports_mutex);
			snapshot = g_sender_audio_lod_exports;
		}
		const int count = std::min<int>(max_count, static_cast<int>(snapshot.size()));
		for (int i = 0; i < count; ++i)
		{
			const SenderAudioLodExportState &source = snapshot[static_cast<std::size_t>(i)];
			out_stats[i].index = source.index;
			out_stats[i].enabled = source.enabled;
	...
	UNITY_INTERFACE_EXPORT const char *SSPS_GetLastError()
	{
		std::lock_guard<std::mutex> lock(g_error_mutex);
