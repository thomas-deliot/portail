...
namespace streamproto::sender
{
	...
	inline std::vector<std::string> BuildEncoderCandidates(const SenderOptions &options)
	{
	...
	private:
		...
		bool CreateD3D11HardwareContexts(AVCodecContext *ctx, const SenderOptions &options, std::string &error)
		{
		...
		bool TryOpenEncoder(const std::string &name, const SenderOptions &options, std::string &error)
		{
